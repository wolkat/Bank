    ...
    class Konto
    {
        ...
        public void DokonajWplaty(decimal kwota) 
        {
            if (kwota < 0)
                throw new ArgumentException(
                    "Wpłacaana kwota nie może być mniejsza od zera");
            saldo += kwota;
        }
        ...
        public string DokonajWyplaty(decimal kwota, int pin) 
        {
            if (!SprawdzPin(pin) || saldo < kwota)
                return "Operacaja anulowana";
            saldo -= kwota;
            return "Operacjęzakończono pomyślnie";
        }
        ...
        public string PobierzInformacje(int pin) 
        {
            if (SprawdzPin(pin))
                return string.Format(
                    "Pan(i) {0} {1} posiada na koncie: {2}", Wlasciciel.Imie, Wlasciciel.Nazwisko, saldo);
            return "Brak dostępu";
        }
